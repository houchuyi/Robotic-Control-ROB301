...
class BayesLoc:
    ...
    def __init__(self, color_map, delivery):
	...
	self.rvl = [color[clr],np.where(self.p[PID.visited] == prob)[0][0]+1,prob, PID.visited ]
	print(self.rvl)
	...
	if self.measured_rgb[0] < self.measured_rgb[1] and self.measured_rgb[2] < 10: 
		...
		return 3
	elif self.measured_rgb[2] > self.measured_rgb[1] and self.measured_rgb[2] > 190 and self.measured_rgb[0] < 70:
		return 0
	else:
		return 4
...
if __name__=="__main__":
    ...
    BL=BayesLoc(color_map,delivery)
    rospy.sleep(0.5)
    rate=rospy.Rate(50)
